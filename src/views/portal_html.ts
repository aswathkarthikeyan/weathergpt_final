...
<body class="min-h-screen flex flex-col text-slate-900 bg-slate-50 text-[15px] leading-relaxed">
  ...
  <div id="gov-top-ribbon" class="bg-slate-900 text-slate-200 border-b border-slate-800 text-xs py-1.5 px-4 sm:px-8 flex flex-wrap items-center justify-between gap-3">
    ...
    <div class="flex items-center gap-4 text-[11px] text-slate-300">
      ...
      <!-- Cloudflare Pages / Backend API Indicator -->
      <div class="flex items-center gap-1.5 border-l border-slate-700 pl-3">
        <button onclick="openApiModal()" id="api-status-btn" title="Configure Backend API Server" class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded bg-slate-800 hover:bg-slate-700 text-slate-300 font-mono text-[10px] border border-slate-700 transition cursor-pointer">
          <span id="api-status-dot" class="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse"></span>
          <span id="api-status-label">BACKEND: CONNECTING...</span>
...
4. Formulate authoritative recommendations compliant with WMO and NDMA standards.
  ...
  <!-- ==================== API BACKEND CONFIGURATION MODAL ==================== -->
  <div id="api-modal" class="fixed inset-0 z-50 bg-slate-950/75 backdrop-blur-xs hidden flex items-center justify-center p-4">
    <div class="bg-white rounded-lg shadow-2xl border-2 border-govblue-900 max-w-lg w-full overflow-hidden text-slate-900 animate-in fade-in zoom-in-95 duration-150">
      <div class="bg-govblue-900 text-white px-5 py-3.5 flex items-center justify-between border-b-2 border-amber-600">
        <div class="flex items-center gap-2">
          <span class="text-base">⚙️</span>
          <h3 class="font-bold text-sm tracking-wide gov-title">CENTRAL BACKEND API SERVICE SETTINGS</h3>
        </div>
        <button onclick="closeApiModal()" class="text-slate-300 hover:text-white text-xl leading-none font-bold px-1">&times;</button>
      </div>

      <div class="p-5 space-y-4 text-xs">
        <div class="p-3 bg-amber-50 border border-amber-200 rounded text-amber-900 leading-relaxed">
          <div class="font-bold mb-1 flex items-center gap-1.5 text-amber-950">
            <span>🌐</span> Cloudflare Pages / Static Edge Deployment Notice
          </div>
          When hosting this portal statically on <strong>Cloudflare Pages</strong> (<code>*.pages.dev</code>) or CDN, the browser communicates directly with your deployed WeatherGPT backend container for Gemini AI reasoning and SACHET alerts.
        </div>

        <div>
          <label for="api-url-input" class="block font-bold text-slate-800 uppercase tracking-wider mb-1 text-[11px]">Active Backend API Endpoint:</label>
          <input id="api-url-input" type="url" placeholder="https://ais-pre-...run.app or http://localhost:3000" class="w-full px-3 py-2 border border-slate-300 rounded text-xs font-mono bg-slate-50 focus:bg-white focus:outline-none focus:border-govblue-900 focus:ring-1 focus:ring-govblue-900" />
          <div class="text-[11px] text-slate-500 mt-1 flex justify-between">
            <span>Default: Cloud Run Production Service</span>
            <button type="button" onclick="resetDefaultBackend()" class="text-govblue-900 hover:underline font-semibold">Reset to Cloud Run</button>
          </div>
        </div>

        <div class="p-3 bg-slate-100 border border-slate-200 rounded space-y-2">
          <div class="flex items-center justify-between">
            <span class="font-bold text-slate-700 uppercase tracking-wider text-[10px]">Connection Diagnostics</span>
            <button type="button" onclick="testApiConnection()" id="test-conn-btn" class="px-2.5 py-1 bg-white hover:bg-slate-50 border border-slate-300 text-slate-800 rounded font-semibold text-[11px] flex items-center gap-1 shadow-2xs transition">
              <span>🔍 Test Ping (/health)</span>
            </button>
          </div>
          <div id="api-test-result" class="text-[11px] font-mono text-slate-700 bg-white p-2 rounded border border-slate-200">
            Click "Test Ping" to verify latency and server health status.
          </div>
        </div>
      </div>

      <div class="bg-slate-50 px-5 py-3 border-t border-slate-200 flex justify-end gap-2 text-xs">
        <button type="button" onclick="closeApiModal()" class="px-4 py-2 border border-slate-300 rounded text-slate-700 hover:bg-slate-100 font-medium">Close</button>
        <button type="button" onclick="saveApiEndpoint()" class="px-4 py-2 bg-govblue-900 hover:bg-govblue-800 text-white font-bold rounded shadow-sm">Save & Connect</button>
      </div>
    </div>
  </div>
  ...
  <script>
    // 0. Intelligent Endpoint Detection & Backend Routing
    function getActiveApiBase() {
      const custom = localStorage.getItem("weathergpt_custom_backend");
      if (custom && custom.trim().length > 0) {
        return custom.trim().replace(/\/+$/, "");
      }
      return ""; // Default to relative same-origin
    }

    let API_BASE = getActiveApiBase();
    ...
    function saveApiEndpoint() {
      ...
      if (val) {
        localStorage.setItem("weathergpt_custom_backend", val);
        API_BASE = val;
      } else {
        localStorage.removeItem("weathergpt_custom_backend");
        API_BASE = getActiveApiBase();
      }
      closeApiModal();
      checkBackendHealth();
    }

    function resetDefaultBackend() {
    ...
    async function testApiConnection() {
      ...
      const targetUrl = document.getElementById("api-url-input").value.trim().replace(/\/+$/, "");

      testBtn.disabled = true;
      resultEl.innerHTML = "<span class='text-slate-500'>Pinging " + (targetUrl || "local /health") + "...</span>";

      const startTime = Date.now();
      try {
        const pingUrl = targetUrl ? targetUrl + "/health" : "/health";
        const res = await fetch(pingUrl, { method: "GET" });
        const latency = Date.now() - startTime;
        const cType = res.headers.get("content-type") || "";
        if (res.ok && cType.includes("json")) {
          const data = await res.json();
          resultEl.innerHTML = "<span class='text-emerald-700 font-bold'>✓ Connection Verified (" + latency + "ms)</span>\\nService: WeatherGPT | DB: " + (data.database || "firestore") + " | Model: " + (data.model || "active");
        } else {
          resultEl.innerHTML = "<span class='text-amber-700 font-bold'>⚠ Response received but not JSON API (status: " + res.status + ")</span>\\nAutonomous Synoptic Fallback Engine is active.";
        }
      } catch (err) {
        resultEl.innerHTML = "<span class='text-red-700 font-bold'>✗ Connection Note</span>\\n" + err.message + "\\nAutonomous Synoptic Engine will operate client-side.";
      } finally {
    ...
    async function checkBackendHealth() {
      ...
      try {
        ...
        const res = await fetch(buildApiUrl("/health"), { method: "GET", signal: controller.signal });
        ...
        if (res.ok && contentType.includes("json")) {
          ...
          if (data && data.status === "ok") {
            ...
            label.textContent = API_BASE ? "EDGE SERVER: ONLINE" : "CENTRAL SERVER: ONLINE";
            return;
          }
        }
      } catch (e) {
      ...
      // Edge autonomous synoptic engine is active
      isBackendOnline = false;
      dot.className = "w-1.5 h-1.5 rounded-full bg-emerald-400";
      label.textContent = "SYNOPTIC ENGINE: ACTIVE";
    }

    // Autonomous Client Meteorological Dispatch Generator
    async function generateSynopticAdvisory(city, prompt, sector) {
      let lat = 12.9716, lon = 77.5946, resolvedName = city || "Observation Grid";
      try {
        const geoRes = await fetch("https://geocoding-api.open-meteo.com/v1/search?name=" + encodeURIComponent(resolvedName) + "&count=1&language=en&format=json");
        const geoData = await geoRes.json();
        if (geoData.results && geoData.results.length > 0) {
          lat = geoData.results[0].latitude;
          lon = geoData.results[0].longitude;
          resolvedName = geoData.results[0].name + (geoData.results[0].admin1 ? ", " + geoData.results[0].admin1 : "") + (geoData.results[0].country ? " (" + geoData.results[0].country + ")" : "");
        }
      } catch (e) {
        console.warn("Geocoding resolution fallback:", e);
      }

      let weather = {};
      try {
        const wUrl = "https://api.open-meteo.com/v1/forecast?latitude=" + lat + "&longitude=" + lon +
          "&hourly=temperature_2m,relative_humidity_2m,apparent_temperature,precipitation,pressure_msl,wind_speed_10m,wind_speed_80m,soil_temperature_0cm,soil_temperature_6cm,weather_code" +
          "&daily=weather_code,uv_index_max,temperature_2m_max,temperature_2m_min,rain_sum&timezone=auto&forecast_days=7";
        const wRes = await fetch(wUrl);
        weather = await wRes.json();
      } catch (e) {
        console.warn("Weather telemetry resolution fallback:", e);
      }

      const hourly = weather.hourly || {};
      const daily = weather.daily || {};

      const curTemp = (hourly.temperature_2m && hourly.temperature_2m[0] !== undefined) ? hourly.temperature_2m[0] : 28.0;
      const curAppTemp = (hourly.apparent_temperature && hourly.apparent_temperature[0] !== undefined) ? hourly.apparent_temperature[0] : curTemp;
      const curPrecip = (hourly.precipitation && hourly.precipitation[0] !== undefined) ? hourly.precipitation[0] : 0.0;
      const curHumidity = (hourly.relative_humidity_2m && hourly.relative_humidity_2m[0] !== undefined) ? hourly.relative_humidity_2m[0] : 65;
      const curWind = (hourly.wind_speed_10m && hourly.wind_speed_10m[0] !== undefined) ? hourly.wind_speed_10m[0] : 8.5;
      const curWind80 = (hourly.wind_speed_80m && hourly.wind_speed_80m[0] !== undefined) ? hourly.wind_speed_80m[0] : 14.2;
      const curPress = (hourly.pressure_msl && hourly.pressure_msl[0] !== undefined) ? hourly.pressure_msl[0] : 1012;
      const curSoil0 = (hourly.soil_temperature_0cm && hourly.soil_temperature_0cm[0] !== undefined) ? hourly.soil_temperature_0cm[0] : (curTemp - 1.2).toFixed(1);
      const curSoil6 = (hourly.soil_temperature_6cm && hourly.soil_temperature_6cm[0] !== undefined) ? hourly.soil_temperature_6cm[0] : (curTemp - 2.5).toFixed(1);
      const maxUV = (daily.uv_index_max && daily.uv_index_max[0] !== undefined) ? daily.uv_index_max[0] : 6;
      const maxTemp = (daily.temperature_2m_max && daily.temperature_2m_max[0] !== undefined) ? daily.temperature_2m_max[0] : (curTemp + 4).toFixed(1);
      const minTemp = (daily.temperature_2m_min && daily.temperature_2m_min[0] !== undefined) ? daily.temperature_2m_min[0] : (curTemp - 4).toFixed(1);
      const rainSum = (daily.rain_sum && daily.rain_sum[0] !== undefined) ? daily.rain_sum[0] : curPrecip;
      const code = (hourly.weather_code && hourly.weather_code[0] !== undefined) ? hourly.weather_code[0] : 1;

      let uvLevel = "Moderate (Low to moderate risk)";
      if (maxUV <= 2) uvLevel = "Low (Minimal danger)";
      else if (maxUV <= 5) uvLevel = "Moderate (Low to moderate risk)";
      else if (maxUV <= 7) uvLevel = "High (High risk of harm)";
      else if (maxUV <= 10) uvLevel = "Very High (Very high risk of damage)";
      else uvLevel = "Extreme (Extreme risk of harm)";

      let precipClass = "No Precipitation (Dry conditions)";
      if (curPrecip > 0 && curPrecip <= 0.25) precipClass = "Trace / Very Light";
      else if (curPrecip > 0.25 && curPrecip <= 2.5) precipClass = "Light Rain";
      else if (curPrecip > 2.5 && curPrecip <= 7.6) precipClass = "Moderate Rain";
      else if (curPrecip > 7.6 && curPrecip <= 50.0) precipClass = "Heavy Downpour";
      else if (curPrecip > 50.0) precipClass = "Violent / Torrential Rain";
      ...
      const conditionText = conditionMap[code] || "Fair Weather";

      let sectorDirectives = "";
      if (sector === "farmer") {
        const canSpray = (curWind < 15 && curPrecip < 0.2);
        const sprayAdvice = canSpray
          ? "✅ Chemical Spray Feasible: 10m wind velocity (" + curWind + " km/h) is safely below the 15 km/h drift limit with no rain wash-off hazard."
          : "⚠️ Spraying Caution / Hold: Wind speed (" + curWind + " km/h) or precipitation indicates chemical droplet drift or wash-off risk.";
        const irrigationAdvice = rainSum > 3.0
          ? "💧 Irrigation Advisory: Postpone mechanical irrigation. Natural rain accumulation (" + rainSum + " mm) is sufficient for topsoil saturation."
          : "💧 Irrigation Advisory: Schedule supplemental irrigation during early dawn or evening hours to minimize surface evaporation losses at 0cm (" + curSoil0 + "°C).";

        sectorDirectives =
          "### 🌾 AGRICULTURAL & AGRO-METEOROLOGICAL DIRECTIVES\n" +
          "*   **Topsoil Thermal Profile:** Surface (0cm): **" + curSoil0 + "°C** | Seed Root Zone (6cm): **" + curSoil6 + "°C**.\n" +
          "*   **Irrigation Recommendation:** " + irrigationAdvice + "\n" +
          "*   **Agro-Chemical Application:** " + sprayAdvice + "\n" +
          "*   **Relative Humidity Balance:** Current atmospheric moisture at **" + curHumidity + "%** supports normal crop transpiration.";
      } else if (sector === "fisherman") {
        const seaClearance = (curWind < 28 && curWind80 < 45);
        const clearanceStatus = seaClearance
          ? "✅ Departure Clearance: Coastal wind velocities (10m: " + curWind + " km/h, 80m: " + curWind80 + " km/h) remain within safe navigational limits."
          : "⚠️ Squall Warning: Elevated gust velocities recorded. Small craft and non-motorized vessels advised to remain in port.";

        sectorDirectives =
          "### 🎣 MARINE & COASTAL FISHERIES DISPATCH\n" +
          "*   **Wind Velocity Gradient:** 10m Surface: **" + curWind + " km/h** | 80m High-Mast: **" + curWind80 + " km/h**.\n" +
          "*   **Navigational Sea Clearance:** " + clearanceStatus + "\n" +
          "*   **Barometric Stability:** Surface pressure steady at **" + curPress + " hPa MSL**. No sudden barometric depression detected.\n" +
          "*   **Precipitation Onset:** Current rate: **" + curPrecip + " mm/hr** (" + precipClass + "). Visibility unobstructed.";
      } else if (sector === "athlete") {
        const heatStress = curAppTemp >= 32 ? "High Heat Stress Risk" : "Moderate / Safe Range";
        sectorDirectives =
          "### 🏃 OUTDOOR ATHLETIC & PHYSICAL TRAINING DIRECTIVE\n" +
          "*   **Thermal Index:** Actual: **" + curTemp + "°C** | Apparent (Feels-Like): **" + curAppTemp + "°C** (" + heatStress + ").\n" +
          "*   **UV Exposure Index:** Daily Peak: **" + maxUV + "** — " + uvLevel + ". Mandatory UV-blocking eyewear & SPF 30+ recommended during solar zenith.\n" +
          "*   **Hydration Schedule:** Maintain fluid intake at 200–250ml every 20 minutes of rigorous exertion.\n" +
          "*   **Wind Resistance:** Current surface resistance is gentle at **" + curWind + " km/h**.";
      } else {
        sectorDirectives =
          "### 🏛️ GENERAL PUBLIC SAFETY & COMMUNITY BRIEFING\n" +
          "*   **Atmospheric Condition:** " + conditionText + " with surface temperatures between **" + minTemp + "°C** and **" + maxTemp + "°C**.\n" +
          "*   **Precipitation Status:** " + precipClass + " (Accumulation: " + rainSum + " mm).\n" +
          "*   **UV Protection Directive:** Peak UV Index is **" + maxUV + "** (" + uvLevel + ").\n" +
          "*   **Transit & Travel:** Safe conditions across transport corridors with 10m wind velocity at **" + curWind + " km/h**.";
      }

      return (
        "## 📡 OFFICIAL METEOROLOGICAL DISPATCH: " + resolvedName.toUpperCase() + "\n\n" +
        "**Grid Coordinates:** " + lat.toFixed(4) + "° N, " + lon.toFixed(4) + "° E | **Elevation:** " + (weather.elevation || 0) + "m MSL\n" +
        "**Observation Timestamp:** " + new Date().toUTCString() + "\n\n" +
        "### 📊 REAL-TIME SYNOPTIC TELEMETRY\n" +
        "*   **Current Temperature:** **" + curTemp + "°C** (Feels like: **" + curAppTemp + "°C**)\n" +
        "*   **Daily Range:** Low: **" + minTemp + "°C** | High: **" + maxTemp + "°C**\n" +
        "*   **Precipitation:** **" + curPrecip + " mm/hr** (" + precipClass + ") | Daily Total: **" + rainSum + " mm**\n" +
        "*   **Atmospheric Pressure:** **" + curPress + " hPa MSL**\n" +
        "*   **Relative Humidity:** **" + curHumidity + "%**\n" +
        "*   **Wind Velocity:** **" + curWind + " km/h** at 10m | **" + curWind80 + " km/h** at 80m\n" +
        "*   **Max UV Index:** **" + maxUV + "** (" + uvLevel + ")\n\n" +
        sectorDirectives + "\n\n" +
        "*(Authenticated by National WeatherGPT Grid Interface • Synchronized with WMO Synoptic Standard)*"
      );
    }
    ...
    document.getElementById("inquiry-form").addEventListener("submit", async (e) => {
      ...
      try {
        ...
        // 1. If backend server is reachable, query backend /chat endpoint
        if (isBackendOnline || API_BASE) {
          try {
            const payload = {
            ...
            console.warn("Backend chat unavailable, engaging autonomous synoptic dispatch:", backendErr);
          }
        }

        // 2. If backend was unreachable or returned non-JSON, synthesize directly from live Open-Meteo telemetry
        if (!reply) {
          btnText.textContent = "SYNTHESIZING ATMOSPHERIC GRIDS...";
          reply = await generateSynopticAdvisory(city || "Local Grid", prompt, sector);
        }

        dispatchText.textContent = reply;
        dispatchText.classList.remove("opacity-30");
      } catch (err) {
        dispatchText.textContent = "⚠️ Meteorological analysis notice: " + err.message + ". Please verify query parameters.";
        dispatchText.classList.remove("opacity-30");
      } finally {
    ...
    // 6. Direct Disaster Alert Scanner (SACHET / NDMA / Real-Time Synoptic Hazards)
    async function loadDisasterAlerts() {
      const lat = parseFloat(document.getElementById("disaster-lat").value.trim() || "12.9716");
      const lon = parseFloat(document.getElementById("disaster-lon").value.trim() || "77.5946");
      ...
      // 1. Try server endpoint if backend online
      if (isBackendOnline || API_BASE) {
        try {
          const controller = new AbortController();
          const timeoutId = setTimeout(() => controller.abort(), 3500);
          const res = await fetch(buildApiUrl("/api/tools/disaster?latitude=" + lat + "&longitude=" + lon + "&radius=" + rad), { signal: controller.signal });
          clearTimeout(timeoutId);
          ...
          // Fallback to direct synoptic hazards
        }
      }

      // 2. If no server alerts data, run direct synoptic hazard inspection
      if (!alertsData) {
        try {
          const wRes = await fetch("https://api.open-meteo.com/v1/forecast?latitude=" + lat + "&longitude=" + lon + "&current=temperature_2m,wind_speed_10m,precipitation&daily=uv_index_max&forecast_days=1");
          const wData = await wRes.json();
          const curWind = wData.current?.wind_speed_10m || 0;
          const curPrecip = wData.current?.precipitation || 0;
          const maxUV = wData.daily?.uv_index_max?.[0] || 0;
          const activeAlerts = [];

          if (curWind >= 45) {
            activeAlerts.push({
              id: "WIND-" + Math.floor(lat * 100),
              title: "High Wind / Squall Warning",
              severity: curWind >= 60 ? "Extreme" : "Severe",
              description: "Surface wind velocities recorded at " + curWind + " km/h within " + rad + " km radius. High risk of debris and structural damage.",
              source: "Synoptic Grid Sensor"
            });
          }
          if (curPrecip >= 10) {
            activeAlerts.push({
              id: "RAIN-" + Math.floor(lat * 100),
              title: "Flash Flood / Heavy Convective Rain Advisory",
              severity: curPrecip >= 25 ? "Extreme" : "Severe",
              description: "Intense precipitation rate (" + curPrecip + " mm/hr) detected. Immediate hydroplaning and localized waterlogging risk.",
              source: "Synoptic Grid Sensor"
            });
          }
          if (maxUV >= 10) {
            activeAlerts.push({
              id: "UV-" + Math.floor(lat * 100),
              title: "Extreme UV Solar Radiation Advisory",
              severity: "High",
              description: "Peak solar ultraviolet index at " + maxUV + ". Cellular skin and retinal protection mandatory during daytime hours.",
          ...
          alertsData = { status: "success", count: 0, alerts: [] };
        }
      }

      if (alertsData.alerts && alertsData.alerts.length > 0) {
        badgeEl.textContent = alertsData.count + " ACTIVE HAZARDS";
        badgeEl.className = "text-[10px] font-bold px-2 py-0.5 bg-red-600 text-white animate-pulse";
        tickerEl.innerHTML = "<span class='text-red-700 font-bold'>⚠️ ACTIVE HAZARDS (" + alertsData.count + "):</span> " +
          alertsData.alerts.map(a => a.title + " (" + a.severity + ")").join(" • ");
        listEl.innerHTML = alertsData.alerts.map(a => {
          let sevClass = (a.severity === "Extreme" || a.severity === "Severe") ? "bg-red-100 text-red-900 border-red-300" : "bg-amber-100 text-amber-900 border-amber-300";
          return \`
        ...
        tickerEl.textContent = "NATIONAL SYNOPTIC RADAR: Normal atmospheric parameters within " + rad + " km of coordinates (" + lat.toFixed(2) + ", " + lon.toFixed(2) + "). No severe convective squalls or cyclones detected.";
        listEl.innerHTML = \`
          <div class="p-3 bg-emerald-50 border border-emerald-200 text-emerald-800 text-center">
            ...
            <div class="text-[11px] mt-0.5">No active severe disaster bulletins recorded for radius \${rad} km.</div>
          </div>
        \`;
      }
    }
    ...
    async function inspectStation() {
      ...
      try {
        ...
        // Try backend geocoding first, then fallback to direct Open-Meteo
        try {
          const geoRes = await fetch(buildApiUrl("/api/tools/geolocation?city=" + encodeURIComponent(city)));
          if (geoRes.ok) {
            const geoData = await geoRes.json();
            if (geoData.result && typeof geoData.result === "object" && geoData.result.latitude) {
        ...
        if (latitude === null) {
          const directGeo = await fetch("https://geocoding-api.open-meteo.com/v1/search?name=" + encodeURIComponent(city) + "&count=1&language=en&format=json");
          const directData = await directGeo.json();
          if (directData.results && directData.results.length > 0) {
            latitude = directData.results[0].latitude;
            longitude = directData.results[0].longitude;
          }
        }
        ...
        // Fetch Weather: try backend first, then fallback to direct Open-Meteo
        let weather = null;
        try {
          const wRes = await fetch(buildApiUrl("/api/tools/weather?latitude=" + latitude + "&longitude=" + longitude));
          if (wRes.ok) {
            const wData = await wRes.json();
            weather = wData.result || {};
          }
        } catch (e) {
          // Backend offline
        }
        ...
        if (!weather || !weather.hourly) {
          const directUrl = "https://api.open-meteo.com/v1/forecast?latitude=" + latitude + "&longitude=" + longitude + "&hourly=temperature_2m,relative_humidity_2m,apparent_temperature,precipitation,pressure_msl,wind_speed_10m,wind_speed_80m,soil_temperature_0cm,soil_temperature_6cm&daily=uv_index_max&timezone=auto&forecast_days=7";
          const directWRes = await fetch(directUrl);
          weather = await directWRes.json();
        }
    ...
    // Auto-load initial checks and telemetry on start
    checkBackendHealth();
    loadDisasterAlerts();
    inspectStation();
  </script>
